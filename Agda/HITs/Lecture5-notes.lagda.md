...
mult-funext : S1 → S1 → S1
mult-funext = S1-rec ((\ y → y)) (λ≡ (S1-elim (λ z → z ≡ z) loop (PathOver-path-loop (refl _))))
```
...
```agda
mult : S1 → S1 → S1
mult x y = S1-rec y (S1-elim (λ z → z ≡ z) loop (PathOver-path-loop (refl _)) y) x
```
